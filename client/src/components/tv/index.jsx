  ...
  return (
    <button
      ...
      className={`w-10 h-10 flex items-center justify-center rounded-full touch-manipulation transition-colors ${
        active  ? 'bg-white/[0.15] text-white/80' :
        danger  ? 'text-white/35 hover:bg-danger/10 hover:text-danger/70' :
...
function FlatBtn({ children, onClick }) {
  return (
    <button
      onClick={onClick}
      className="w-10 h-9 rounded-item bg-white/[0.08] text-white/50 flex items-center justify-center touch-manipulation hover:bg-white/[0.14] active:bg-white/[0.20] transition-colors"
    >
      {children}
    </button>
  );
...
function RoundBtn({ children, onClick, dim }) {
  return (
    <button
      onClick={onClick}
      className={`w-12 h-12 rounded-full flex items-center justify-center touch-manipulation transition-colors ${
        dim
...
export default function TV() {
  ...
  return (
    <div className="relative h-full flex flex-col">
      ...
      {/* ── Top bar ── */}
      <div className="shrink-0 flex items-center gap-2 px-4 py-2 border-b border-subtle">

        {/* Now Playing + Keyboard — left group */}
        ...
        {/* Device controls — right group, separated by divider */}
      ...
      {remoteOpen && (
        <>
          ...
          {/* Bottom-left: D-pad */}
          <div className="absolute bottom-4 left-4 z-20 p-3 rounded-2xl bg-black/50 backdrop-blur-sm" data-no-swipe>
            <div className="relative w-36 h-36">
              <div className="absolute inset-0 rounded-full bg-white/[0.06]" />
              <button onClick={() => key('Up')}
                className="absolute top-0 left-1/2 -translate-x-1/2 w-11 h-11 flex items-center justify-center text-white/60 hover:text-white/90 touch-manipulation transition-colors">
                <ChevronUpIcon size={22} />
              </button>
              <button onClick={() => key('Down')}
                className="absolute bottom-0 left-1/2 -translate-x-1/2 w-11 h-11 flex items-center justify-center text-white/60 hover:text-white/90 touch-manipulation transition-colors">
                <ChevronDownIcon size={22} />
              </button>
              <button onClick={() => key('Left')}
                className="absolute left-0 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center text-white/60 hover:text-white/90 touch-manipulation transition-colors">
                <ChevronLeftIcon size={22} />
              </button>
              <button onClick={() => key('Right')}
                className="absolute right-0 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center text-white/60 hover:text-white/90 touch-manipulation transition-colors">
                <ChevronRightIcon size={22} />
              </button>
              <button onClick={() => key('Select')}
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-white/[0.14] text-white/70 hover:bg-white/[0.22] active:bg-white/[0.30] flex items-center justify-center touch-manipulation transition-colors">
          ...
          <div className="absolute bottom-4 right-4 z-20 p-3 rounded-2xl bg-black/50 backdrop-blur-sm flex flex-col gap-2.5" data-no-swipe>
            <div className="flex justify-around gap-2">
              <FlatBtn onClick={() => key('Back')}><BackIcon /></FlatBtn>
              <FlatBtn onClick={() => key('Home')}><HomeIcon size={18} /></FlatBtn>
            </div>
            <div className="flex items-center gap-2">
              <FlatBtn onClick={() => key('Rev')}><ScanBackIcon size={18} /></FlatBtn>
              <RoundBtn onClick={() => key('Play')}>
                {isPlaying ? <PauseIcon /> : <PlayIcon size={22} />}
              </RoundBtn>
              <FlatBtn onClick={() => key('Fwd')}><ScanFwdIcon size={18} /></FlatBtn>
            </div>
            <div className="flex items-center gap-2">
              <FlatBtn onClick={() => key('VolumeDown')}><VolumeDownIcon size={18} /></FlatBtn>
              <RoundBtn onClick={() => key('VolumeMute')} dim><VolumeMuteIcon size={20} /></RoundBtn>
              <FlatBtn onClick={() => key('VolumeUp')}><VolumeUpIcon size={18} /></FlatBtn>
            </div>
          </div>
        </>
      )}
